...
class OdooSyncRun(models.Model):
    ...
    def _process(self, deadline):
        ...
        try:
            while self.state == 'running':
                self._step(engine)
                done += self._save(engine, done)
                self.invalidate_recordset(['state'])
                if self.phase == 'done' or time.monotonic() > deadline:
                    break
    ...
    def _save(self, engine, done):
        ...
        processed = stats.get('processed', 0) + stats.get('relinked', 0)
        pending = 0 if self.state in ('done', 'failed', 'cancelled') else max(self.total - self.processed, 0) + 1
        self.env['ir.cron']._notify_progress(done=done + processed, remaining=pending)
        self.env.cr.commit()
        return processed
